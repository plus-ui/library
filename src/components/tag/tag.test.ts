...
describe('PlusTag Component', () => {
  ...
  it('applies status variant correctly', async () => {
    const el = await fixture(html`<plus-tag status="success">Success Tag</plus-tag>`);
    expect(el.shadowRoot?.querySelector('div')?.classList.contains('bg-color-status-success-default')).to.be.true;
  });

  it('applies size variant correctly', async () => {
    const el = await fixture(html`<plus-tag size="lg">Large Tag</plus-tag>`);
    expect(el.shadowRoot?.querySelector('div')?.classList.contains('text-md')).to.be.true;
  });

  it('applies radius variant correctly', async () => {
    const el = await fixture(html`<plus-tag radius="medium">Medium Radius Tag</plus-tag>`);
    expect(el.shadowRoot?.querySelector('div')?.classList.contains('rounded-md')).to.be.true;
  });

  it('applies invert variant correctly', async () => {
    const el = await fixture(html`<plus-tag invert>Inverted Tag</plus-tag>`);
    expect(el.shadowRoot?.querySelector('div')?.classList.contains('invert')).to.be.true;
  });

  it('sets aria-label correctly', async () => {
    const el = await fixture(html`<plus-tag aria-label="Custom Label">Tag with Custom Label</plus-tag>`);
    expect(el.shadowRoot?.querySelector('div')?.getAttribute('aria-label')).to.equal('Custom Label');
  });
});